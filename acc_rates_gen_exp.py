...
def acc_rates(params):
    ...
    test = np.vstack((xfunc.dat.data, yfunc.dat.data,  u0.dat.data[:,0])).T
    test = test[(-test[:,1]).argsort()]
    test = test.reshape((65,97,3))
    for j in range(65):
        curr = np.vstack((test[j,:,0], test[j,:,1], test[j,:,2])).T
    ...
    for step in tqdm.trange(num_timesteps):
        ...
        if step==num_timesteps-1:
            ...
            colors = icepack.plot.tripcolor(f, cmap="RdBu", axes=axes)
            fig.colorbar(colors, label="meters/year", fraction=0.012, pad=0.04)
            plt.savefig("mass_end.png")
            plt.close()

    return ice_stream_sol   

# samples = 20
# friction_rate = np.linspace(0.5, 2.5, samples)
# friction_in = np.linspace(3, 5, samples)
...
#             params = [friction_in[i], friction_rate[j]]
#             ice_stream_sol[sol_index,:,:,:] = acc_rates(params)
#             sol_index+=1

# np.save(f"/home/ak2152@ad.eng.cam.ac.uk/mnt/Code/fourier_neural_operator-master/inverse/friction_line.npy", ice_stream_sol)
