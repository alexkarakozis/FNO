...
# samples = 10
# melt_rates = np.linspace(0, 1, samples)
# fluidity  = np.linspace(245,265, samples)


# ice_stream_sol = np.zeros((samples**2, 65,97,4))

# sol_index = 0 
# for i in tqdm.trange(samples):
#         for j in range(samples):
#             ice_stream_sol[sol_index,:,:,:] = acc_rates([melt_rates[i], fluidity[j]])
#             sol_index += 1

# np.save(f"/home/ak2152@ad.eng.cam.ac.uk/mnt/Code/fourier_neural_operator-master/inverse/melt_fluidity.npy", ice_stream_sol)
...
np.save(f"observed_friction_noisy_4.5.npy", noisy_data)
