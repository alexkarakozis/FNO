...
def init_cond(melt_rate):
    ...
    m = melt_rate[0] # melt_rate
    expr = (0.95 - 0.05 * x / Lx) * τ_D / u_in**(1 / m)
    C = firedrake.interpolate(expr, Q)
    ice_stream_sol[:,:,0] =  C .dat.data.reshape(65,97)
...
def fourier(melt_rate):
    ...
    with open('a_normalizer_friction.pkl', 'rb') as f:
        a_normalizer = pickle.load(f)

    y_normalizer = 0
    # Load the object from the file
    with open('y_normalizer_friction.pkl', 'rb') as f:
    ...
    index = 0
    model = torch.load("/home/ak2152@ad.eng.cam.ac.uk/mnt/Code/fourier_neural_operator-master/model/friction_ep500", map_location=device)
...
# # Load noisy data
y = np.load("/home/ak2152@ad.eng.cam.ac.uk/mnt/observed_friction_noisy.npy")
# y = y[::2,::2,:]
# y = y[:65,:97,:]
observed_data = y
print(y.shape)
...
np.save("/home/ak2152@ad.eng.cam.ac.uk/mnt/mcmc_samples_sliding_exp_solver_thousand.npy", pcn_u)
print(pcn_acc)
