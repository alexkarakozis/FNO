...
def init_cond(melt_rate):
    ...
    A = firedrake.Constant(melt_rate[0])
    # ice_stream_sol[:,:,0] =  firedrake.interpolate(firedrake.Constant(melt_rate), Q).dat.data.reshape(65,97)
    ...
    # ice_stream_sol[:,:,0] = a.dat.data.reshape(65,97)
...
def fourier(melt_rate):
    ...
    with open('a_normalizer_fluidity.pkl', 'rb') as f:
        a_normalizer = pickle.load(f)

    y_normalizer = 0
    # Load the object from the file
    with open('y_normalizer_fluidity.pkl', 'rb') as f:
    ...
    index = 0
    model = torch.load("/home/ak2152@ad.eng.cam.ac.uk/mnt/Code/fourier_neural_operator-master/model/fluidity_ep500", map_location=device)
...
# # Load noisy data
y = np.load("/home/ak2152@ad.eng.cam.ac.uk/mnt/observed_fluidity_noisy.npy")
observed_data = y

# Covariance
fluidity_prior = 4
N = 1
K = np.diag([fluidity_prior])
...
np.save("/home/ak2152@ad.eng.cam.ac.uk/mnt/mcmc_samples_rate_factor_solver_thousand.npy", pcn_u)
print(pcn_acc)