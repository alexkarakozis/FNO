...
def init_cond(melt_rate):
    ...
    ice_stream_sol = np.zeros((65,97, 1))

    a_in = firedrake.Constant(1.7)
    δa = firedrake.Constant(-2.7)
    expr =  a_in + δa * x / Lx - conditional(x / Lx > 0.5, melt_rate, 0.0)
    ...
    # cp = plt.imshow(ice_stream_sol[:,:,0])
    # plt.colorbar(cp)
    # plt.axis('off')
    # plt.show()
...
def fourier(melt_rate):
    ...
    t1 = default_timer()
    data = torch.tensor(init_cond(melt_rate[0]), dtype=torch.float)
    ...
    with open('a_normalizer_melt_correct.pkl', 'rb') as f:
        a_normalizer = pickle.load(f)

    y_normalizer = 0
    # Load the object from the file
    with open('y_normalizer_melt_correct.pkl', 'rb') as f:
    ...
    pred = torch.zeros(10, S1, S2, 1)

    index = 0
    model = torch.load("/home/ak2152@ad.eng.cam.ac.uk/mnt/Code/fourier_neural_operator-master/model/melt_rate_correct_ep500", map_location=device)
...
def log_likelihood(y, f, sigma):
    ...
    M = f.shape[2]
    for j in range(M):
        l2_total = l2_total + math.pow(rel(y[:,:,j], f),2)
    
    return -0.5*l2_total/(M*math.pow(sigma,2))

def pcn(N, u0, y, n_iters, beta, Kc, melt_mean_prior, sigma):
    ...
    for i in notebook_tqdm.trange(n_iters):
        # z = np.random.randn(N, )
        z = np.random.randn(N)
        u_new = np.sqrt(1 - pow(beta, 2)) * u_prev + beta * (Kc @ z + melt_mean_prior) # Propose new sample using pCN proposal
...
# y = np.load("/home/ak2152@ad.eng.cam.ac.uk/mnt/observed_melt.npy")
observed_data = y
print(y.shape)
# Covariance
melt_mean_prior = 0
...
# Run MCMC
pcn_u, pcn_acc = pcn(N, u0, observed_data, n_iters, beta, Kc, melt_mean_prior, sigma)
pcn_u = np.array(pcn_u)
np.save("/home/ak2152@ad.eng.cam.ac.uk/mnt/mcmc_samples_melt_solver_thousand.npy", pcn_u)
print(pcn_acc)