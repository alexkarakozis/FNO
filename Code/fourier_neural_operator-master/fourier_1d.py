...
for x, y in test_loader:
    ...
    fig, ax = plt.subplots()
    ax.plot(x[0], label='Input initial conditions u(x,$t$) where $t \in$ [0,1024]')
    ax.plot(pred[index], label='Predicted time evolution of initial conditions u(x,t) where $t \in$ [1025,2048]')
    ax.plot(y[0], 'k--', label='Actual time evolution of initial conditions u(x,t) where $t \in$ [1025,2048]')
    index += 1
